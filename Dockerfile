...
RUN apt-get update && \
	...
	apt-get -y install --no-install-recommends gcc-8 g++-8 default-libmysqlclient-dev libpq-dev libseccomp-dev ruby-dev ant libluajit-5.1-2 curl unzip mariadb-server screen redis-server jq && \
	cd /tmp && \
